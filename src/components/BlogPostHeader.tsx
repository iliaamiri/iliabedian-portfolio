    ...
    return (
        <div className={`-mt-20 md:-mt-40 flex flex-col w-full items-center`}>
            ...
            <div className={`-mt-5 md:-mt-20 w-11/12 md:w-9/12`}>
                ...
                <div className={`flex w-full justify-end mb-5 items-center gap-4 pr-10`}>
                    <Link target={"_blank"} className={`hover:opacity-50 mr-2`} href={`https://twitter.com/intent/tweet?url=${currentUrl}&text=${blog.title}`}>
                        <Image src={Icon_Twitter} alt={'Share on Twitter'} width={40} height={40} className={`cursor-pointer`} />
                    </Link>
                    <Link target={"_blank"} className={`hover:opacity-50`} href={`https://www.linkedin.com/shareArticle?mini=true&url=${currentUrl}&title=${blog.title}`}>
                        <Image src={Icon_Linkedin} alt={'Share on LinkedIn'} width={30} height={30} className={`cursor-pointer`} />
                    </Link>
                    <button className={`hover:opacity-50`} onClick={() => navigator.clipboard.writeText(currentUrl || "")}>
                        <Image src={Icon_CopyLink} alt={'Copy link'} width={45} height={45} className={`cursor-pointer`} />
                    </button>
                </div>
            </div>
        </div>
