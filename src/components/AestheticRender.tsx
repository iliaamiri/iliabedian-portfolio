...
export const AestheticRender = (
  ...
  for (let i = 0; i < seed; i++) {
    ...
    render.push(
      <div
        ...
        style={{
          backgroundColor: `${
            i % rand == 0 ? '#5441FF' : i % (rand + 1) ? '#3C9069' : '#5441FF'
          }`,
          width: `${randomIntFromInterval(30, 150)}px`,
        }}
        className={`h-[0.6rem] w-fit shrink-0 rounded-full`}
      ></div>
    );
  }
  ...
  return (
    <div className={`relative`}>
      ...
      <div
        className={`absolute top-0 flex flex-row gap-3 ${speedToAnimation[speed][1]}`}
      >
        {render}
      </div>
    </div>
