...
export function FancierSyntaxHighlighter({className, language, ...props}: any) {
    ...
    return (
        <div className={`relative`}>
                <span onClick={handleCopy} className={`absolute -top-4 md:top-1 -right-4 md:right-1 bg-[#6C5CFF] md:bg-transparent text-white px-2 py-1 rounded-full hover:bg-[#5441FF] cursor-copy transition-all ease-in-out duration-300`}>
                    {copied
