    ...
    return (
        <div className={`flex flex-col md:flex-row w-full max:h-[1100px] md:max-h-[708px] overflow-hidden justify-end`}>
            ...
            <div className={`flex-col flex justify-start items-start w-full md:w-7/12 pt-10`}>
                <div className={`w-full md-w-11/12 flex flex-col justify-center items-center hover:scale-[1.03] ease-in-out duration-300`}>
                    <div
                        style={{backgroundImage: `url(${artifact.imageUrl})`}}
                        className={`flex justify-center w-11/12 md:w-9/12 rounded-3xl justify-self-center aspect-video bg-no-repeat bg-cover`}>
                    </div>
                    <div className={`relative -top-20 flex flex-col w-11/12 md:w-9/12 text-[#F9F4F1] bg-gradient-to-t from-[#0A0A0A] via-[#22246A] rounded-b-2xl p-9 pt-20`}>
                        <h1 className={`text-[#52B788] text-3xl ${rowdiesBold.className}`}>
                            {artifact.name}
                            <XSThreeDots
            ...
            <div className={`flex flex-row items-center w-full md:w-5/12 max-h-[580px] justify-center pt-5 md:pt-60`}>
                <div className={`relative w-fit self-center justify-self-center`}>
                    <div className={`absolute flex flex-col justify-between top-0 w-[400px] h-[400px] shrink-0 grow-0 rotate-45 overflow-hidden rounded-2xl bg-primaryDawn z-10`}>
                        <div>
                            <SkillBadgeRow technologies={artifact.technologies[0].list} shouldMarquee={artifact.technologies[0].shouldMarquee} />
                            <SkillBadgeRow technologies={artifact.technologies[1].list} shouldMarquee={artifact.technologies[1].shouldMarquee} />
                        </div>
                        ...
                        <div className={`flex gap-10 justify-center pb-10`}>
                            <ArtifactActionButton size={'medium'} className={`after:content-githubIcon relative z-10 button-view-on-github`}>View on GitHub</ArtifactActionButton>
                            <ArtifactActionButton size={'medium'} className={`after:content-['🚀'] relative z-10 button-view-demo`}>View Demo</ArtifactActionButton>
                        </div>
                    </div>
                    <div className={`relative top-[75px] right-6 bg-opacity-70 w-[350px] h-[350px] shrink-0 grow-0 rotate-45 rounded-2xl bg-primaryDawn`}>
                    </div>
                    <div style={{
                        top: '-200px',
...
function ArtifactItemOnLeft({artifact}: { artifact: ArtifactModel }) {
    ...
    return (
        <div className={`flex flex-col-reverse md:flex-row w-full max:h-[1100px] md:max-h-[708px] overflow-hidden justify-end`}>

            {/* diamond (rotated div) */}
            <div className={`flex flex-row items-center w-full md:w-5/12 max-h-[580px] justify-center pt-5 md:pt-60`}>
                <div className={`relative w-fit self-center justify-self-center`}>
                    <div className={`absolute flex flex-col justify-between top-0 w-[400px] h-[400px] shrink-0 grow-0 -rotate-[35deg] overflow-hidden rounded-2xl bg-primaryDawn z-10`}>
                        <div>
                            <SkillBadgeRow technologies={artifact.technologies[0].list} shouldMarquee={artifact.technologies[0].shouldMarquee} />
                            <SkillBadgeRow technologies={artifact.technologies[1].list} shouldMarquee={artifact.technologies[1].shouldMarquee} />
                        </div>
