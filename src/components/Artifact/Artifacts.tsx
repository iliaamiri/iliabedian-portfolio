...
const useArtifacts = (category: string | undefined) => {
  ...
  const filteredArtifacts = Object.values(artifacts).filter(
    (a) =>
      a.categories.includes(ArtifactCategory.All) ||
...
export function Artifacts({
  ...
  return (
    <Suspense fallback={<Loading />}>
      <StyledHeader id={'artifacts'}>{title}</StyledHeader>
      <div
        className={`grid w-full grid-cols-1 items-stretch justify-items-center gap-5 px-5 lg:grid-cols-2 xl:grid-cols-3`}
      >
        {render}
      </div>
    </Suspense>
