...
Go to your `Program.cs` and add these two lines before `var app = builder.Build();` line. This will allow Railway to
...
<Note type='warning'>
Comment out the line that forces HTTPS redirection. () in your `Program.cs`.
```csharp
...
0. Navigate to `/MyApp` folder in your command line shell.
1. Run `railway login` and follow the instructions to login on your railway account.
2. (optional) run `railway run dotnet run --project ./MyApp` and make sure that your app is running (local)
...
5. On you terminal, (staying in the `/MyApp` working directory), run `railway link` and then choose your project that you want to link your .NET app to. (There will be a list of your railway projects by their names.)
